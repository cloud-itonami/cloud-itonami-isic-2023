(ns soapmfg.operation-test
  "Smoke tests for the compiled SoapOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `soapmfg.governor-contract-test`; the Store contract in
  `soapmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "SoapOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "kettle-001" :severity :moderate :description "caustic vapor concentration rising near saponification kettle"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
