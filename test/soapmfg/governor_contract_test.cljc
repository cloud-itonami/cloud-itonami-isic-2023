(ns soapmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the saponification/mixing/
  formulation kettle or filling line directly... does NOT authorize
  or execute maintenance/line operations') implemented faithfully.
  The single invariant under test:

    SoapAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Soap & Detergent Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :bar-soap}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :bar-soap (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-saponification-kettle :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-soap}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-soap (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-off-spec-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:off-spec-rate-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-off-spec-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:off-spec-rate-percent (store/batch db "batch-001"))) "fabricated off-spec-rate never lands in the SSoT")))

(deftest fragrance-allergen-labeling-incomplete-is-held
  (testing "a fragrance-bearing batch's patch that cites allergens without completing labeling disclosure -> HOLD"
    (let [[db actor] (fresh)
          res (exec-op actor "t11b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:fragrance-allergens [:eugenol :coumarin]}} coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:fragrance-allergen-labeling-incomplete} (-> (store/ledger db) last :basis)))
      (is (= [:limonene :linalool] (:fragrance-allergens (store/batch db "batch-001")))
          "the incomplete allergen patch never lands in the SSoT -- batch-001 keeps its seeded allergens ([:eugenol :coumarin] never merged in)"))))

(deftest fragrance-allergen-labeling-complete-auto-commits
  (testing "a fragrance-bearing batch's patch that cites allergens AND declares labeling complete -> auto-commits cleanly"
    (let [[db actor] (fresh)
          res (exec-op actor "t11c" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:fragrance-allergens [:eugenol :coumarin]
                                             :allergen-labeling-complete? true}} coordinator)]
      (is (= :commit (get-in res [:state :disposition])))
      (is (= [:eugenol :coumarin] (:fragrance-allergens (store/batch db "batch-001")))))))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "kettle-001" :severity :moderate
                                            :description "caustic vapor concentration rising near saponification kettle"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "kettle-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
