(ns soapmfg.advisor
  "SoapAdvisor -- the *contained intelligence node* for the soap-and-
  detergent, cleaning-and-polishing-preparation, perfume, and toilet-
  preparation (cosmetics) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/weight/off-
  spec-rate/fragrance-allergens), drafts a saponification/mixing/
  formulation-kettle or filling-line maintenance scheduling proposal
  ...
  a committed record and NEVER a real formulation/filling-line
  actuation or freight dispatch. Every output is censored downstream
  by `soapmfg.governor` before anything touches the SSoT -- see
  ...
  contract every caller of this actor agrees to) -- `soapmfg.governor`
...
(defn- log-production-batch
  ...
  weight, off-spec-rate, fragrance-allergens, or verification status.
...
(defn- schedule-maintenance
  "Draft a saponification/mixing/formulation-kettle or filling-line
  ...
  registered?) in its rationale, but `soapmfg.governor` NEVER trusts
  this report -- it independently re-derives verified?/registered?
  from the equipment's own stored fields before any commit is
  possible."
...
(defn- flag-safety-concern
  "Draft a chemical-hazard/allergen-labeling/microbial-contamination
  ...
  technicality). See `soapmfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `soapmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound soap/detergent/cleaning-preparation/perfume/
  toilet-preparation shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed weight -- it does NOT invent one, and `soapmfg.governor`
  NEVER trusts it: it independently recomputes whether the batch's
  own cumulative-shipped weight plus this claim would exceed the
  batch's own recorded weight before any commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは石鹸・洗剤・洗浄/研磨剤・香水・化粧品(toilet preparation)工場"
       ...
       "サポニフィケーション釜・配合設備・充填ライン等の設備の直接操作(actuate)を"
       "絶対に提案してはいけません(この actor は提案のみを行い、実行は一切"
       "行いません)。出荷量を偽って報告してはいけません。香料アレルゲンを含む"
       "配合について、ラベル開示の完了を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `soapmfg.governor`
