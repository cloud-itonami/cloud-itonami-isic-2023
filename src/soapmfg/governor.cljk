(ns soapmfg.governor
  "Soap & Detergent Plant Operations Governor -- the independent
  compliance layer that earns the SoapAdvisor the right to commit.
  ...
  draft-schedule) the saponification/mixing/formulation kettle or
  filling line, whether a shipment proposal's own claimed weight would
  blow through the batch's own logged production weight, whether a
  fragrance-bearing batch's own allergen disclosure is actually
  complete, or when an act stops being a coordination proposal and
  becomes direct formulation/filling-line-equipment control, so this
  MUST be a separate system able to *reject* a proposal and fall back
  to HOLD.

  `:itonami.blueprint/governor` is
  `:soap-detergent-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:kettle/actuate`
                                       or `:line/run`) is the 'direct
                                       formulation/filling-line-
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       saponification/mixing/
                                       formulation kettle or filling
   ...
   10. Invalid off-spec-rate       -- for `:log-production-batch`, if
                                       ...
                                       reading
                                       (`soapmfg.registry/off-spec-
                                       rate-valid?`), the batch record
                                       is rejected rather than let
                                       fabricated/sensor-error data
                                       through.
   11. Fragrance-allergen labeling
       incomplete                  -- for `:log-production-batch`,
                                       INDEPENDENTLY re-derive whether
                                       the EFFECTIVE product type
                                       (patch's own `:product-type`, or
                                       else the batch's already-
                                       recorded type) ordinarily bears
                                       a fragrance AND the patch cites
                                       one or more designated fragrance
                                       allergens
                                       (`soapmfg.registry/fragrance-
                                       allergen-labeling-incomplete?`)
                                       -- if so, the patch's own
                                       `:allergen-labeling-complete?`
                                       flag MUST independently be
                                       `true`. EU Regulation (EC) No
                                       1223/2009 Annex III disclosure
                                       obligation, never taken on the
   ...
   12. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [soapmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  filling-line-equipment-control effect."
...
(defn- line-control-blocked-violations
  ...
  Anything else (direct saponification/mixing/formulation-kettle or
  filling-line control, a fabricated actuation effect) is this actor's
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はサポニフィケーション釜・配合設備・充填ラインの直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the saponification/mixing/formulation kettle or
  filling line -- this actor may only ever propose/schedule a DRAFT
  maintenance window, never actuate the line directly. No override,
  ever."
  [{:keys [op]} proposal]
...
(defn- invalid-off-spec-rate-violations
  ...
  `:off-spec-rate-percent` that is not a physically plausible reading,
  reject rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rr (:off-spec-rate-percent (:value proposal))]
      (when (and (some? rr) (not (registry/off-spec-rate-valid? rr)))
        [{:rule :invalid-off-spec-rate
          :detail (str rr "% は物理的に妥当な不良率の範囲外")}]))))

(defn- fragrance-allergen-labeling-incomplete-violations
  "For `:log-production-batch`, INDEPENDENTLY re-derive the EFFECTIVE
  product type (patch's own `:product-type`, else the batch's
  already-recorded type from the store) and check whether it
  ordinarily bears a fragrance AND the patch cites one or more
  designated fragrance allergens -- if so, the patch's own
  `:allergen-labeling-complete?` flag MUST independently be `true`.
  EU Regulation (EC) No 1223/2009 Annex III disclosure obligation,
  ...
  (when (= op :log-production-batch)
    (let [patch (:value proposal)
      ...
      (when (registry/fragrance-allergen-labeling-incomplete? effective-product-type patch)
        [{:rule :fragrance-allergen-labeling-incomplete
          :detail (str subject " (product-type=" effective-product-type
                       ") の香料アレルゲン開示(" (pr-str (:fragrance-allergens patch))
                       ")に対し :allergen-labeling-complete? が true でない -- EU 1223/2009 附属書III開示義務")}]))))

(defn check
  "Censors a SoapAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (fragrance-allergen-labeling-incomplete-violations request proposal st)))
