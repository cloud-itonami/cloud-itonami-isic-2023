(ns soapmfg.store
  "SSoT for the soap-and-detergent, cleaning-and-polishing-preparation,
  perfume, and toilet-preparation (cosmetics) plant-operations
  ...
  Decision 1, this vertical is self-contained (no external soap/
  detergent/cosmetics-manufacturing capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/weight/off-spec-rate/
                             fragrance-allergen record. `:verified?`
    ...
    - `equipment`         -- a saponification/mixing/formulation-kettle
                             or filling-line unit's own record.
    ...
    - `maintenance`       -- a scheduled saponification/mixing/
                             formulation-kettle or filling-line
                             maintenance-window DRAFT against a piece
                             of equipment (`soapmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound soap/detergent/
                             cleaning-preparation/perfume/toilet-
                             preparation shipment DRAFT
                             (`soapmfg.registry`'s `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (soapmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (soapmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :bar-soap :output-form :bar
                :material "Coconut-Oil Milled Bar Soap"
                :weight-kg 5000.0 :off-spec-rate-percent 1.1
   ...
   "batch-002" {:id "batch-002" :product-type :liquid-soap :output-form :liquid
                :material "Glycerin-Base Liquid Hand Soap"
                :weight-kg 8000.0 :off-spec-rate-percent 0.8
                :fragrance-allergens [:geraniol]
                :allergen-labeling-complete? true
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :laundry-detergent :output-form :powder
                :material "Enzyme-Active Laundry Detergent Powder"
                :weight-kg 20000.0 :off-spec-rate-percent 2.0
                :fragrance-allergens []
                :allergen-labeling-complete? false
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"kettle-001" {:id "kettle-001" :kind :saponification-kettle
                 :verified? true :registered? true
                 :last-maintenance-date "2026-05-01"}
   "filler-002" {:id "filler-002" :kind :filling-line
                 :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `soapmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `soapmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- two verified+registered batches with allergen-labeling
  already complete (one with shipping headroom, one nearly fully
  shipped so a small new shipment blows through its own logged
  weight -- HARD hold), one UNVERIFIED/unregistered batch with an
  incomplete allergen disclosure (blocks any shipment coordinated
  against it); one verified+registered saponification-kettle unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  filling-line unit (blocks any maintenance scheduling against it) --
