(ns soapmfg.sim
  ...
  saponification/mixing/formulation kettle or filling line directly
  ...
  off-spec-rate reading, and a production-batch patch declaring
  fragrance allergens without a completed allergen-labeling
  disclosure.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on kettle-001 (verified, registered, saponification-kettle -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on kettle-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-saponification-kettle :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on filler-002 (UNVERIFIED/unregistered filling line -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on kettle-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 declaring fragrance allergens without completing allergen-labeling disclosure -> HARD hold ==")
