(ns soapmfg.operation
  "SoapOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (SoapAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Soap & Detergent Plant
...
(defn build
  "Compiles a SoapOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `soapmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SoapAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Soap & Detergent Plant Operations Governor -- independent censor.
      (g/add-node :govern
